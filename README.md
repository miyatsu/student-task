...
首页入口也已调整为更偏“工作台入口”而不是“功能陈列”的结构：首屏从上到下依次堆叠为 Hero 价值说明、PDF / Image / Word capability strip、以及 Workspace Upload 面板。Hero 标题上方直接显示 `Local-first by default` 与 `AI only when configured` 两个 trust pills，主标题文案为 `PDF, Word, and Images in One Local-First Workspace`；副标题则拆成两句独立文案，第二句更新为 `LLM stays off until you configure a key.`。实际上传入口只保留在最下方的 Workspace Upload 面板中，面板保留明确的 **Choose files** 按钮，并把图片格式摘要合并为 `PNG/JPG/JPEG`。
