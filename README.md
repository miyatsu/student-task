...
当前工作区支持更直观的排序胶囊按钮、按名称自然排序、逐文件上移/下移，并且在图片与 Word 批量转 PDF 时都会显示实时绿色进度卡片；Word 转 PDF 进度现在会同时显示已用时长与当前采用的转换方式，完成时会在列表内收束到 `100%` 而不是打断式成功弹窗。高保真 Word 转 PDF 现已默认按 `LibreOffice CLI -> 本地 Microsoft Word -> 浏览器 HTML` 的顺序尝试；旧版 `.doc` 文档也可以通过内置的服务端提取链路转成 PDF。
