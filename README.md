...
首页入口也已调整为更偏“工作台入口”而不是“功能陈列”的结构：首屏从上到下依次堆叠为 Hero 价值说明、PDF / Image / Word capability strip、以及 Workspace Upload 面板。Hero 标题上方直接显示 `Local-first by default` 与 `AI only when configured` 两个 trust pills，主标题文案为 `PDF, Word, and Images in One Local-First Workspace`，并收紧成更适合桌面端单行居中显示的字号；副标题拆成两句独立文案，第二句为 `LLM stays off until you configure a key.`，同时首句末尾的 `and export the result.` 会保持为同一段不拆行。中段三张能力卡片的标题现为 `PDF Workflow`、`Image Workflow`、`Word Workflow`。最下方的 Workspace Upload 面板则回退到更简洁的居中卡片风格：保留顶部上传图标、`Workspace Upload` 标签和格式胶囊，移除大标题与说明段，改为整卡点击上传，并用 `Choose files from your device, or drag and drop them here.` 提示文件对话框与拖拽两种方式。
