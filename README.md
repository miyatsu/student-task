...
首页入口也已调整为更偏“工作台入口”而不是“功能陈列”的结构：首屏直接组合产品价值说明与真实 Workspace Upload 面板，左侧标题上方直接显示 `Local-first by default` 与 `AI only when configured` 两个 trust pills；实际上传入口只保留在右侧 Workspace Upload 面板中，避免同屏重复 CTA。右侧上传卡片当前恢复为带明确 **Choose files** 按钮的 workspace upload 版本，并把图片格式摘要合并为 `PNG/JPG/JPEG`。上传区下方的 capability strip 也改成了图标在左、标题在右的一行摘要结构，并使用更直观的 PDF、图片、Word 文档图标。
