...
首页入口也已调整为更偏“工作台入口”而不是“功能陈列”的结构：首屏直接组合产品价值说明与真实 Workspace Upload 面板，左侧标题上方直接显示 `Local-first by default` 与 `AI only when configured` 两个 trust pills，用户可以在第一屏直接选择文件或拖拽上传，再往下查看 PDF / Image / Word 三类核心能力说明。
