...
4. **办公文本分析**：
   ...
   - [可选] 将各类图文重组转换。
5. **集成式 AI 对话助手**：
   - 能够基于上述步骤产生的纯文本结果和图文流，利用外挂的 `@google/genai` (Gemini API) 进行连贯的问答与总结交互。
6. **多文件归档与打包下载**：在用户操作完毕后，能对最终产物执行 Zip 通用压缩处理，减少零散下载。

## 非功能需求

1. **隐私与安全保护**：图像增强算法和文档拆分合并**必须**全程在浏览器前台运行（依托浏览器环境），除了用户明确需要通过 Gemini API 生成回答的情况，禁止把用户的个人文件上传到第三方服务器。
