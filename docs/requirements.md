...
4. **办公文本分析**：
   ...
   - [可选] 将各类图文重组转换。
   - 对于图片文字提取，应优先走本地 PaddleOCR 离线能力，而不是把图片上传到云端 OCR provider。
...
5. **集成式 AI 对话助手**：
   ...
   - AI 助手与图片 OCR 应解耦：AI key 只用于对话与分析，不应成为图片 OCR 的前置条件。
6. **多文件归档与打包下载**：在用户操作完毕后，能对最终产物执行 Zip 通用压缩处理，减少零散下载。

## 非功能需求

1. **隐私与安全保护**：图像增强算法、图片 OCR，以及文档拆分合并**必须**全程在本地环境运行（浏览器前台或本机自带服务进程）；除了用户明确需要通过已配置的云端 AI provider 生成回答的情况，禁止把用户的个人文件上传到第三方服务器。
