...
最近一轮微调里，Workspace Upload 卡片回退到了更早一版的居中式视觉骨架：顶部是圆形上传图标，其下只保留 `Workspace Upload` 标签、格式胶囊以及一条次级拖拽提示 `or drag and drop files here`。原先的 `Drop files into your workspace` 大标题与长说明段被移除，格式摘要也更新为 `PDF`、`DOC / DOCX`、`PNG / JPG / JPEG` 这组更紧凑的写法；唯一的主操作则保留为底部钢蓝色的 **Choose files** 按钮。
