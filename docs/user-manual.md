...
当前首页已经改为“工作台入口”结构：首屏左侧说明产品价值，左上角直接展示 `Local-first by default` 与 `AI only when configured` 两个状态标签，右侧就是实际的 Workspace Upload 面板。你既可以点击 Hero 区域的 **Choose files**，也可以直接点击或拖拽到上传面板里；两者复用同一套上传逻辑，不会产生两个分离的入口。
