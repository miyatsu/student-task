...
当前首页已经改为“工作台入口”结构：从上到下依次是产品价值说明、三张能力摘要卡片、以及实际的 Workspace Upload 面板，但这三段内容已被进一步收进同一个首屏外壳中，其中能力摘要卡片与上传区共享一块内层面板，从视觉上更接近一个连续的工作台入口。首页顶部直接展示 `Local-first by default` 与 `AI only when configured` 两个状态标签；Hero 区的状态标签、主标题、两句说明文字和主 CTA 现在共用一条中心轴，主按钮 **Choose files** 会直接打开文件选择器并沿用现有上传流程。说明文字第二句为 `LLM stays off until you configure a key.`，同时第一句末尾的 `and export the result.` 会保持为连续短语，不再单独掉到下一行。当前首页只保留最下方上传面板这一个上传入口；上传卡片现在被收窄成更紧凑的上传区，保留上传图标、`Workspace Upload` 标签和格式胶囊，但改用明确的 **Choose files** 按钮作为主操作，并把拖拽方式降为次级提示 `or drag and drop files here`。格式胶囊当前显示为 `PDF`、`DOC / DOCX`、`PNG`、`JPG / JPEG`；而中间的能力区则进一步收束为一条共享 ribbon，三项标题分别为 `PDF Workflow`、`Image Workflow`、`Word Workflow`，并统一使用同一套文件徽章风格。最终样式又统一到了更暖的中性色背景、成熟一些的阴影层级、深蓝强调色以及本地优先的 sans 字体栈。
