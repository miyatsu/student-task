...
当前首页已经改为“工作台入口”结构：首屏左侧说明产品价值，左上角直接展示 `Local-first by default` 与 `AI only when configured` 两个状态标签，右侧就是实际的 Workspace Upload 面板。当前首页只保留右侧上传面板这一个上传入口，你可以直接点击面板或拖拽文件到面板里开始处理。上传区下方的三张能力卡片则改成“图标在左、标题在右”的摘要样式，更容易快速扫读 PDF / Image / Word 三条工作流。
