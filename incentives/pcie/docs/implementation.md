...
1. `local Microsoft Word`（`word-com`）
2. `LibreOffice CLI`（`libreoffice-cli`）
3. `browser HTML fallback`

前端通过 `convertWordFileToPdfBlob()` 按上述顺序依次尝试：先调用服务端 `/api/word/convert-pdf` 尝试原生后端（`word-com` / `libreoffice-cli`），由 [src/server/word-pdf-native.ts](../src/server/word-pdf-native.ts) 负责后端探测与 PDF 导出执行。当原生后端均不可用时，回退到浏览器 HTML 渲染链路。Python 包装方案如 `docx2pdf`、`pywin32`、UNO / `unoconv` 本质上仍是对 Word 或 LibreOffice 的调用，项目不做内置集成。
...
6. [src/server/word-pdf-native.ts](../src/server/word-pdf-native.ts)：负责原生 Word→PDF 转换后端的探测、COM/LibreOffice 调用与输出验证。
