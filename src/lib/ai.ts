...
You can enable AI by configuring any one of these environment variables:
...
export async function loadAiRuntimeConfig() {
  if (cachedRuntimeConfig && isAiConfigured(cachedRuntimeConfig)) {
    return cachedRuntimeConfig;
  }

  if (!runtimeConfigPromise) {
    runtimeConfigPromise = fetchRuntimeConfig().then((runtimeConfig) => {
      cachedRuntimeConfig = isAiConfigured(runtimeConfig) ? runtimeConfig : undefined;
...
function buildLocalAiGatewayErrorMessage(actionLabel: string) {
  return `${actionLabel} failed because the local AI gateway could not be reached. Ensure this workspace is running through npm run dev or npm run start, then retry.`;
}
...
async function postJson<TResponse>(
  ...
  try {
    ...
    if (!message || message === 'Failed to fetch' || message === 'fetch failed') {
      throw new Error(buildLocalAiGatewayErrorMessage(actionLabel));
    }
...
export async function extractImageTextWithAi(file: AiRequestFile) {
  const response = await postJson<AiTextResponse>('/api/ai/ocr', { file }, 'AI OCR request');
  return response.text;
}